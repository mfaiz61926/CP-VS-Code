...
void m_conq() {
        ...
        for(int total_R = 0; total_R <= n; total_R++) {
            vector<vector<int>> dp(n + 1, vector<int>(n + 1, INT_MAX));
            dp[0][0] = 0;
            
            for(int pos = 1; pos <= n; pos++) {
                for(int r = 0; r <= pos; r++) {
                    if(dp[pos-1][r] == INT_MAX) continue;
                    
                    {
                        int r_new = r;
                        if(r >= k) {
                            int cost = dp[pos-1][r] + (s[pos-1] != 'L');
                            dp[pos][r_new] = min(dp[pos][r_new], cost);
                        }
                    }
                    
                    {
                        int r_new = r + 1;
                        int l_after = (n - pos) - (total_R - r_new);
                        if(l_after >= k) {
                            int cost = dp[pos-1][r] + (s[pos-1] != 'R');
                            dp[pos][r_new] = min(dp[pos][r_new], cost);
                        }
                    }
                }
            ...
            if(dp[n][total_R] != INT_MAX) {
                ans = min(ans, dp[n][total_R]);
            }
        }
