...
const int N = 200005;
int f(vi & a, vi & b, int i, int j){
    int n = a.size();
    if(i==n) return 0;
    int cnt = 0;
    int take = a[i] + f(a, b, i+1, j);
    int nt = b[j] + f(a, b, i, j+1);
...
int solve(int i, int k) {
    ...
    int pickRed = solve(i + 1, k - a[i]);
    int pickBlue = solve(i + 1, b[i] - k);
    return dp[key] = max(pickRed, pickBlue);
}
using ll = long long;
void solve(){
        int n;
        ...
        long long mn = 0, mx = 0;
        for (int i = 0; i < n; i++) {
            long long mn1 = min(mn - a[i], b[i] - mn);
            long long mx1 = max(mx - a[i], b[i] - mx);
            long long mn2 = min(mn - a[i], b[i] - mn);
            long long mx2 = max(mn - a[i], b[i] - mn);
            // Actually let's compute all four:
            long long c1 = mn - a[i];
            long long c2 = b[i] - mn;
            long long c3 = mx - a[i];
            long long c4 = b[i] - mx;
