...
void m_conq() {
        ...
        for (int &x : b) x = lower_bound(v.begin(), v.end(), x) - v.begin();
        ...
        auto ok = [&](int k) {
            ...
            for (int l = 0; l < n; l += len) {
                ...
                for (int i = l; i < min(n, l + len); i++) {
                    if (!cnt[a[i]]) used.push_back(a[i]);
                    cnt[a[i]]++;
                }

                for (int i = l; i < min(n, l + len); i++) {
                    if (!cnt[b[i]]) used.push_back(b[i]);
                    cnt[b[i]]--;
                }

                for (int x : used) {
                    if (cnt[x]) {
                        for (int y : used) cnt[y] = 0;
                        return false;
                    }
                    cnt[x] = 0;
                }
