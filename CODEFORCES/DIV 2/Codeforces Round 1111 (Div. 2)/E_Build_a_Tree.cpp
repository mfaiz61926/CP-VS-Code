...
void m_conq() {
    ...
    long long mn = 2LL*(n-1);
    long long mx = ((long long)n*n)/2;
    ...
    long long tt = (k - mn) / 2;
    long long m_max = (mx - mn) / 2;
    long long x = m_max - tt;
    ...
    for(int i = 0; i < xi; i++) a[i] = i+1;
    for(int i = 0; i < xi; i++) a[n-xi+i] = n-xi+1+i;
    
    int s = n - 2*xi;
    int lowCnt = (s+1)/2;
    int highCnt = s/2;
    
    vector<int> lowV(lowCnt), highV(highCnt);
    for(int i = 0; i < lowCnt; i++) lowV[i] = xi + 1 + i;
    for(int i = 0; i < highCnt; i++) highV[i] = xi + 1 + lowCnt + i;
    
    int pos = xi;
    int li = 0, hi_ = 0;
    while(li < lowCnt || hi_ < highCnt){
        if(li < lowCnt) a[pos++] = lowV[li++];
        if(hi_ < highCnt) a[pos++] = highV[hi_++];
    }
