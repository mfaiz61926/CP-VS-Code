...
void m_conq() {
    ...
    if (blocks > 2 * min(cnt0, cnt1) + (cnt0 != cnt1)) {
        cout << -1 << endl;
        return;
    }
    ...
    if ((cnt0 && zeroBlocks == 0) || (cnt1 && oneBlocks == 0)) {
        cout << -1 << endl;
        return;
    }

    bool start0 = (zeroBlocks >= oneBlocks);
    ...
    if (zeroBlocks) z[0] += cnt0;
    if (oneBlocks) o[0] += cnt1;
    ...
    while (i < zeroBlocks || j < oneBlocks) {
        if (turn0) {
            ans.append(z[i], '0');
            i++;
        } else {
