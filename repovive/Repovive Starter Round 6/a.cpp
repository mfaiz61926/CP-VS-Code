...
bool is_unhappy(int v, int P) {
    int children = adj[v].size() - (P != 0 ? 1 : 0);
    if (children <= 0) return false;
    int bad_children = C[v] - ((P != 0 && a[P] <= a[v]) ? 1 : 0);
...
void dfs2(int u, int p, int curr_score) {
    ans[u] = curr_score;
    for (int v : adj[u]) {
        if (v == p) continue;
        int old_u = is_unhappy(u, 0);
        int old_v = is_unhappy(v, u);
        int new_u = is_unhappy(u, v);
        int new_v = is_unhappy(v, 0);
        
        int nxt_score = curr_score - old_u - old_v + new_u + new_v;
        dfs2(v, u, nxt_score);
    }
}
void m_conq() {
