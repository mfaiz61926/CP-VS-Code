...
void m_conq() {
        ...
        set<char> fs, ft;
        for(auto &i : s) fs.insert(i);
        for(auto &i : t) ft.insert(i);
        ...
        while(q--){
            ...
            bool ok = false, ok1 = true, ok2 = true;
            for(auto & i : w){
                if(fs.find(i) == fs.end()) ok1 = false;
                if(ft.find(i) == ft.end()) ok2 = false;
            }
