...
void dfs(int x, int p) {
	...
	for(auto &y : adj[x]) {
		...
		if(l.size() > 2) l.pop_back();
	}
	int c = adj[x].size();
	if(c >= 2) ans = max(ans, 1LL);
	if(c >= 3) ans = max(ans, l[0]+1LL);
	if(c >= 4) ans = max(ans, l[0]+l[1]+1LL);
	if(c >= 3) dp[x] = max(1LL, dp[x]);
	if(c >= 4) dp[x] = max(dp[x], l[0]+1LL);
}
