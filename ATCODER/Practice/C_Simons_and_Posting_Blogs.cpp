...
void m_conq() {
        ...
        fr0(i, n){
            int v = -1;
            fr0(j, n) if(!used[j]) if(v == -1 || grid[j] < grid[v]) v = j;
            used[v] = 1;
            set<int>st(all(grid[v]));
            for(auto & it : grid[v]) ans.push_back(it);

            fr0(j, n){
                if(!used[j]){
                    vi temp;
                    for(auto &it : grid[j]) if(st.find(it) == st.end()) temp.push_back(it);
                    grid[j] = temp;
                }
            }
        }
