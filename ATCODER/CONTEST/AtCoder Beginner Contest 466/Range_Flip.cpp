...
int vis[N][21][2];
int dp[N][21][2];
int solve(int i, int op, int p,  vector<pair<int,int>>&v){
    if(i == n) {
        if(p == 0) return 0;
        return -1e18;
    }
    if(vis[i][op][p]) return dp[i][op][p];
    vis[i][op][p] = 1;

    int ans = -1e18;
    int cost = (p?v[i].second - v[i].first : 0);
    ans = max(ans, cost + solve(i + 1, op, p, v));

    if(op > 0){
        ans = max(ans, cost + solve(i + 1, op - 1, p ^ 1, v));
    }
    return dp[i][op][p] = ans;
}
void m_conq() {
        cin >> n >> k;
