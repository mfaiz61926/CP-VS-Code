...
inline void pull(int u, int bit) {
    cnt[u] = xr[u] = 0;
    if (int lc = ch[u][0]) {
        cnt[u] += cnt[lc];
        xr[u] ^= xr[lc];
    ...
    if (int rc = ch[u][1]) {
        cnt[u] += cnt[rc];
        xr[u] ^= xr[rc];
        if (cnt[rc] & 1) xr[u] ^= (1 << bit);
    }
}

void ins(int u, int v, int bit) {
    if (bit == B) { cnt[u]++; return; }
    int b = (v >> bit) & 1;
...
void rem(int u, int v, int bit) {
    if (!u) return;
    if (bit == B) { cnt[u]--; return; }
    int b = (v >> bit) & 1;
    if (ch[u][b]) rem(ch[u][b], v, bit + 1);
    pull(u, bit);
}

void sub1(int u, int bit) {
    if (!u || bit == B) return;
    swap(ch[u][0], ch[u][1]);
    if (ch[u][1]) sub1(ch[u][1], bit + 1);
    pull(u, bit);
}
void m_conq() {
    int n, q;
